...
export default function KnowledgeBase() {
        ...
        try {
          ...
          if (r.ok) {
            ...
            const all = [
              ...
              ...(db.books || []),
              ...(db.articles || []),
            ].map(i => ({ ...i, _cat: i.id?.charAt(0) === 'p' ? 'principles' : i.id?.charAt(0) === 'f' ? 'frameworks' : i.id?.charAt(0) === 'g' ? 'guidelines' : i.id?.charAt(0) === 'b' ? 'books' : 'articles' }))
            setResults(all)
            setLoading(false)
            return
          }
  ...
  useEffect(() => {
    ...
    let db = null
    for (const path of paths) {
      try { const r = await fetch(path); if (r.ok) { db = await r.json(); break } } catch {}
    }
    if (!db) return
    {
        let all = [
